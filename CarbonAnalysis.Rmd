...
summary(sc8)
sc8 <- lmer((c)^2 ~ factor(site) + (1|row) + (1|stake) + om + n, data = soil)
summary(sc8)
...
#fitted vs residual
plot(sc7, type=c("p","smooth"), col.line=1)
#scale-location
plot(sc7,
     sqrt(abs(resid(.)))~fitted(.),
     type=c("p","smooth"), col.line=1)
...
lattice::qqmath(sc7)
#residuals vs leverage
plot(sc7, rstudent(.) ~ hatvalues(.))



#fitted vs residual
plot(sc8, type=c("p","smooth"), col.line=1)
#scale-location
plot(sc8,
     sqrt(abs(resid(.)))~fitted(.),
     type=c("p","smooth"), col.line=1)
